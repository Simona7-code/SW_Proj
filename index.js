/*Realizzate l’interfaccia grafica per la funzionalità di ricerca: ad ogni
nuovo carattere la ricerca viene ripetuta visualizzando il numero di
corrispondenze trovate se più di una, altrimenti i dati del libro
corrispondente.*/
...
class Archivio {
  ...
  cerca(titolo) {
    ...
    for (let libro of this.archivio){
      ...
      console.log("titolo passato---->", titolo)
      console.log(titolo_corr)
      console.log( "titolo corrente->", titolo_corr)
      console.log( "vedi se lo trova->", titolo_corr.toLowerCase().includes(titolo.toLowerCase())? "trovato":"non trovato");
      ...
      if (titolo_corr.toLowerCase().includes(titolo.toLowerCase())=== true) {
        trovato++
...
//let element = document.createElement("ricerca")
//element.addEventListener("oninput", Archivio_Pisa.cerca);

document.getElementById("ricerca").addEventListener('oninput', Archivio_Pisa.cerca())
//Se dentro cerca lasci vuoto da problema di leggere undefined
...
input.addEventListener("input", Archivio_Pisa.cerca());*/